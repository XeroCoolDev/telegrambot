...
RUN pnpm approve-builds --yes && pnpm install --frozen-lockfile || pnpm approve-builds --yes && pnpm install
...
WORKDIR /app
COPY package.json pnpm-lock.yaml* ./
RUN pnpm approve-builds --yes && pnpm install --prod --frozen-lockfile || pnpm approve-builds --yes && pnpm install --prod
